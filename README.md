...
Положить `content/chart/2026-07.md` по образцу соседних (frontmatter: title, period, date, positions с rank/prev/artist/track/flags; тело = обзор месяца). `date` — дата публикации: 1-е число СЛЕДУЮЩЕГО месяца (чарт июля → `2026-08-01`); в архиве месяц выпуска показывается из `period`. Дискам — 2-е число месяца после квартала (Q3 → 2026-10-02), чтобы в архиве и RSS Диск стоял выше чарта последнего месяца → push в main → через ~2 мин выпуск на сайте. Поле `playlist_yandex` добавить, когда плейлист создан.
